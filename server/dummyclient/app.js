...
app.get('/send-image-to-realserver', async (req, res) => {
    ...
    if (!imageName) {
        return res.status(400).json({ success: false, message: 'imageName 쿼리 파라미터가 필요합니다.' });
    }
    ...
    try {
        ...
        // Send to realserver
        const realserverResponse = await fetch('http://localhost:3000/api/receive-json', {
            method: 'POST',
            headers: {
        ...
        if (realserverResponse.ok) {
            res.status(200).json({ success: true, message: '이미지 전송 성공', realserverResponse: result });
        } else {
            res.status(realserverResponse.status).json({ success: false, message: '이미지 전송 실패', realserverResponse: result });
        }

    } catch (error) {
        if (error.code === 'ENOENT') {
            res.status(404).json({ success: false, message: `파일을 찾을 수 없습니다: ${imageName}`, error: error.message });
        } else {
...
app.listen(port, () => {
    console.log(`Dummy Client 서버가 http://localhost:${port} 에서 실행 중입니다.`);
    console.log(`이미지 전송을 위해 http://localhost:${port}/index.html 에 접속하여 버튼을 클릭하세요.`);
});